...
class SpecsServiceDict(TypedDict):
    ...
    changed_parameters_of_service: List[
        Tuple[str, DiagService, List[Union[str, int, float]]]
    ]


class SpecsChangesVariants(TypedDict):
...
class Display:
    ...
    def print_dl_changes(self, service_dict: SpecsServiceDict) -> None:
        ...
        if (
            ...
            or service_dict["deleted_services"]
        ...
        if service_dict["new_services"]:
            ...
            rich_print(extract_service_tabulation_data(service_dict["new_services"]))
        ...
        if service_dict["deleted_services"]:
            ...
            rich_print(
                extract_service_tabulation_data(service_dict["deleted_services"])
            )
        if service_dict["changed_name_of_service"][0]:
            ...
            rich_print(
                extract_service_tabulation_data(
                    service_dict["changed_name_of_service"][0]
                )
            )
        if service_dict["changed_parameters_of_service"][0]:
            ...
            changed_param_column = [
                str(x) for x in service_dict["changed_parameters_of_service"][1]
            ]
...
class Comparison(Display):
    ...
    def compare_services(
        self, service1: DiagService, service2: DiagService
    ) -> List[SpecsServiceDict]:
    ...
    def compare_diagnostic_layers(
        self, dl1: DiagLayer, dl2: DiagLayer
    ) -> SpecsServiceDict:
        ...
        for service1 in dl1.services:
            ...
            if service1 not in dl2.services:
                if rq_prefix is None or rq_prefix not in dl2_request_prefixes:
                    ...
                    service_dict["new_services"].append(
                        service1
            ...
            # check whether names of diagnostic services have changed
                ...
                if rq_prefix is None or rq_prefix in dl2_request_prefixes:
                    ...
                    service_dict["changed_name_of_service"][1].append(
                        service2.short_name
                    )
                    ...
                    if detailed_information[
                        ...
                        service_dict["changed_parameters_of_service"][0].append(
                            service1
                        )
    ...
    ) -> SpecsChangesVariants:  # type: ignore[type-arg]
        ...
        for _, dl1 in enumerate(database_new.diag_layers):
            ...
            for _, dl2 in enumerate(database_old.diag_layers):
                ...
                if (
                    ...
                    service_dict: SpecsServiceDict = self.compare_diagnostic_layers(
                        dl1, dl2
                    )
                    if service_dict:
