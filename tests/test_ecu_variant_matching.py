...
def dummy_response(monkeypatch):
    resp = Response(OdxLinkId("dummy_resp", doc_frags), "dummy_resp", None)
...
def identificationService(monkeypatch, dummy_response):
    dummy_req = Request(OdxLinkId("dummy_req", doc_frags), "dummy_req", [])
    odxlinks.update({dummy_req.odx_id: dummy_req})
    
    diagService = DiagService(
        odx_id=OdxLinkId("identService", doc_frags),
...
def supplierService(monkeypatch, dummy_response):
    ...
    diagService = DiagService(
        odx_id=OdxLinkId("supplierService", doc_frags),
...
def ecu_variant_pattern1():
    return EcuVariantPattern(matching_parameters=[
        MatchingParameter(diag_comm_snref="identService", expected_value="1000", out_param_if_snref="id"),
        MatchingParameter(diag_comm_snref="supplierService", expected_value="supplier_A", out_param_if_snref="name")
        ])

@pytest.fixture
def ecu_variant_pattern2():
    return EcuVariantPattern(matching_parameters=[
        MatchingParameter(diag_comm_snref="identService", expected_value="2000", out_param_if_snref="id"),
        MatchingParameter(diag_comm_snref="supplierService", expected_value="supplier_B", out_param_if_snref="name")
        ])

@pytest.fixture
def ecu_variant_pattern3():
    return EcuVariantPattern(matching_parameters=[
        MatchingParameter(diag_comm_snref="supplierService", expected_value="supplier_C", out_param_if_snref="name")
        ])

@pytest.fixture
def ecu_variant_1(identificationService, supplierService, ecu_variant_pattern1):
    return DiagLayer(DIAG_LAYER_TYPE.ECU_VARIANT, OdxLinkId("ecu_variant1", doc_frags), "ecu_variant1", "variant 1", 
...
def ecu_variant_2(identificationService, supplierService, ecu_variant_pattern2):
    return DiagLayer(DIAG_LAYER_TYPE.ECU_VARIANT, OdxLinkId("ecu_variant2", doc_frags), "ecu_variant2", "variant 2", 
                    services=[identificationService, supplierService],  
                    ecu_variant_patterns=[ecu_variant_pattern2],
                    odxlinks=odxlinks)

@pytest.fixture
def ecu_variant_3(identificationService, supplierService, ecu_variant_pattern1, ecu_variant_pattern3):
    return DiagLayer(DIAG_LAYER_TYPE.ECU_VARIANT, OdxLinkId("ecu_variant3", doc_frags), "ecu_variant3", "variant 3", 
                    services=[identificationService, supplierService],  
...
@pytest.mark.parametrize("req_resp_mapping, expected_variant",
    [
        # test if full match of matching parameters is accepted
        ({b'\x22\x10\00': as_bytes({"id": 2000}), b'\x22\x20\00': as_bytes({"name": "supplier_B"})}, "ecu_variant2"),
        # test if partial match of matching parameters is rejected
        ({b'\x22\x10\00': as_bytes({"id": 2000}), b'\x22\x20\00': as_bytes({"name": "supplier_C"})}, "ecu_variant3"),
        # test if first full match is preferred over second match
        ({b'\x22\x10\00': as_bytes({"id": 1000}), b'\x22\x20\00': as_bytes({"name": "supplier_A"})}, "ecu_variant1"),
    ])
def test_ecu_variant_matching(ecu_variant_1, ecu_variant_2, ecu_variant_3, use_cache, req_resp_mapping, expected_variant):
